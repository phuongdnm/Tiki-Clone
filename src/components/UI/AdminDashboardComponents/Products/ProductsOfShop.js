    ...
    const handleSearch = () => {
        if (inputText.length > 0) {
            ...
            switch (filterOptions) {
                case "id":
                    setProducts(allShops.find(shop => shop._id === inputText).products);
                    setIsLoading(false);
                    return ;
                case "name":
                    setProducts(allShops.find(shop => shop.name === inputText).products);
                    setIsLoading(false);
                    return ;
                default:
                    return ""
            ...
            <Grid container>
                {products !== null && products.length > 0?
                    ...
                    <Grid item xs={10} md={10} lg={12} style={{margin: 0}}>
                        <section style={{display: 'flex', alignItems: 'center', justifyContent: 'space-around',  width: '100%', height: '100%'}}>
