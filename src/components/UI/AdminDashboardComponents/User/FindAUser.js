...
const FindAUser = (props) => {
    ...
    const handleSearch = () => {
        if (inputText.length > 0) {
            ...
            switch (filterOptions) {
                case "id":
                    setUsers_(allUsers.filter(user => user._id === inputText));
                    setIsLoading(false);
                    return ;
                case "name":
                    setUsers_(allUsers.filter(user => user.name === inputText));
                    setIsLoading(false);
                    return ;
                case "email":
                    setUsers_(allUsers.filter(user => user.email === inputText));
                    setIsLoading(false);
                    return ;
                default:
                    return ""
    ...
    const UserCardFull = () => {
        allUsers !== null && setCurrentUserProfile(allUsers.find(user => user._id === currentUserId));

        return <Card profile style={{height: "100%", backgroundColor: "rgba(213, 213, 213, 0.1)"}}>
            <CardAvatar profile>
            ...
            <CardBody profile >
                ...
                <p style={{marginTop: '1em'}}>Account created
                    on {Moment(currentUserProfile !== null && currentUserProfile.createdAt).format('MMMM DD, YYYY')}</p>
                </div>
            </CardBody>
        </Card>
    };
    return (
        <div style={{width: '100%'}}>
            ...
            <Grid container>
                ...
                {users_ !== null && users_.length > 0 && showUserCard ?
                    <Grid item xs={10} md={6} lg={4} style={{margin: "2em", marginLeft: '18vw'}}>
                        <UserCardFull/>
                    </Grid> : null
                }
                {
